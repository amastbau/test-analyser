...
def run_full_simulation():
    ...
    tests = [
        TestResult("test_full_checkout_multi_error", "E2E", "build-503", "staging", "STEP: Adding item to cart\nWARN: Connection timed out... FATAL: NullPointerException", "1.3.0", "stage", "AZURE", ["p1"]),
        TestResult("test_mysql_backup_and_verify", "DB-Backup", "build-507", "prod", "STEP: Running backup for mysql-persistent\nSTEP: Verify backup integrity\nFAIL: Checksum mismatch", "1.4.1", "main", "GCP", ["db"]),
        TestResult("test_vm_provisioning_error", "Infra", "build-511", "ci", "STEP: Launching new VM\nERROR: Request failed, permission denied", "1.5.0", "main", "vSphere", ["provisioning"]),
        ...
        TestResult("test_mysql_restore_validation", "DB-Restore", "build-515", "dev", "STEP: Validating application after restore\nERROR: mysql validation failed", "1.5.1", "dev-branch", "AWS", ["db"]),
        TestResult("test_mysql_teardown_flake", "DB-Teardown", "build-516", "staging", "STEP: Tearing down mysql instance\nERROR: mysql cleanup failed. Error: Connection timed out.", "1.5.1", "stage", "AWS", ["db"]),
        TestResult("test_ocp_mysql_deployment", "Deployment", "build-517", "ci", "STEP: Applying ocp-mysql manifest\nFATAL: ocp-mysql deploy failed.", "1.5.1", "main", "OCP", ["db"]),
        TestResult("test_ansible_role_deploy", "Deployment", "build-518", "ci", "STEP: Running ansible playbook\nError during command execution: ansible-playbook error: one or more host failed... use_role\\\":\\\".../roles/ocp-datagrid\\\"", "1.5.1", "main", "OCP_BAREMETAL", ["deployment"]),
        TestResult("test_large_volume_backup", "Backup", "build-519", "prod", "STEP: Backing up large volume\nINFO: backup completed with warnings", "1.5.2", "main", "GCP", ["storage"]),
        TestResult("test_backup_and_post_hook", "Backup", "build-520", "prod", "STEP: Running backup\nINFO: backup successful\nSTEP: Executing post-backup hook\nERROR: Post-backup hook failed", "1.5.2", "main", "GCP", ["storage"]),
        TestResult("test_infra_setup_failure", "Setup", "build-521", "ci", "STEP: Provisioning network\nFATAL: database migration setup failed", "1.5.2", "main", "OCP", ["infra"]),
        TestResult("test_known_bug_snapshot", "Restore", "build-523", "prod", "STEP: Restoring from snapshot\nFATAL: error restoring snapshot oadp-2345", "1.5.3", "main", "AWS", ["restore", "known-issue"]),
        TestResult("test_known_automation_panic", "Framework", "build-524", "ci", "STEP: Initializing test suite\nCRITICAL: automation framework panicked", "1.5.3", "main", "KIND", ["automation-stability"]),
        TestResult("test_unclassified_failure", "GUI", "build-525", "staging", "STEP: Clicking the 'Save' button\nERROR: Unexpected token '<' at position 0", "1.5.3", "stage", "AWS", ["ui"]),
    ]
...
def log_view(request, test_run_id):
    ...
    # Prepare InstructLab data for this specific view
    instructlab_example = {
        "instruction": "Classify the following test failure log based on the provided context.",
